...
class TestContentImageUploadFailure:
    """Test that image upload failure stops post creation."""

    def test_image_upload_failure_raises(self, mock_settings, mock_post_data):
        """Image upload failure raises PublisherError."""
        mock_post_data.content = "![alt](images/missing.png)"
        publisher = WordPressPublisher()
        with pytest.raises(PublisherError):
            publisher.upload_content_images(mock_post_data)
