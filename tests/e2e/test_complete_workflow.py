...
def test_publisher_creates_wordpress_post(mock_post, mock_get, test_env):
    ...
    mock_get.side_effect = [conn_resp, slug_resp, cats, cats_end]
...
def test_complete_job_workflow(mock_post, mock_get, test_env):
    ...
    cats_end = Mock(status_code=200, json=lambda: [])
    mock_get.side_effect = [conn, slug, cats, cats_end]
...
def test_job_failure_and_retry(mock_post, mock_get, test_env):
    ...
    media_ok = Mock(
        status_code=201, json=lambda: {"id": 123, "source_url": "https://test.com/img.jpg"}
    )
    post_fail = Mock(status_code=500)
    post_ok = Mock(
        status_code=201,
        json=lambda: {"id": 456, "link": "https://test.com/post/", "slug": "blog-automation-guide"},
    )
