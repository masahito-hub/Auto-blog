...
def test_complete_job_workflow(mock_post, mock_get, test_env):
    ...
    media_response = Mock(
        status_code=201,
        json=lambda: {"id": 123, "source_url": "https://test.com/image.jpg"}
    )
...
def test_acceptance_criteria_sample_zip_to_draft(test_env):
    ...
    """
    with patch('app.publisher.requests.Session.get') as mock_get, \
        ...
        mock_post.side_effect = [
            ...
            Mock(status_code=201, json=lambda: {
                "id": 456,
                "link": "https://test.com/blog-automation-guide/",
